...
describe("BidChain Test", function () {
    ...
    // 生成 EIP712 签名
    async function getBidSignature(auctionId, bidAmount, bidder,nftAddress) {
      const domain = {
      name: "NFTAuction",
      version: "1",
      chainId: 31337, // 主网链ID
    ...
    describe("should auction process successfully ", async () => {
        ...
        it("should allow bidder to place a valid bid", async function () {
            const { auctionId} = await mintAndCreateAuction();

            const bidAmount = ethers.utils.parseEther("2"); // 竞标金额
            console.log("begin signature");
            // 获取有效签名
            const signature = await getBidSignature(auctionId, bidAmount, bidder,nftAuction.address);
          ...
          it("should revert if the bid signature is invalid", async function () {
            ...
            const invalidSignature = await getBidSignature(auctionId, bidAmount, invalidBidder,nftAuction.address);
          ...
          it("should end the auction", async function () {
            const { auctionId, tokenId} = await mintAndCreateAuction();
          ...
          it("should end the auction and return the nft when no bidders", async function () {
            const { auctionId, tokenId} = await mintAndCreateAuction();
            ...
            // 检查NFT转移
            expect(await nftAuction.ownerOf(tokenId)).to.equal(minter.address);
            console.log("success to return nft for minter:",minter.address);
          });
